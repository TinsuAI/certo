...
test("should build a view model with provenance and quality metadata", () => {
  ...
  assert.equal(viewModel.preferredTextSourceLabel, "OCR recovery");
...
test("should rewrite absolute canonical markdown links to local document routes", () => {
  ...
  const rewritten = rewriteRenderedMarkdownLinks(html, new Map([
    ["/home/vp/workspace/client/barry-CO/docs/legal/canonical/pilot/05-2018-tt-bct-thong-tu-quy-dinh-ve-xuat-xu-hang-hoa.md", "/doc/05-2018-tt-bct-thong-tu-quy-inh-ve-xuat-xu-hang-hoa"],
  ]), "en");

  assert.match(rewritten, /href="\/doc\/05-2018-tt-bct-thong-tu-quy-inh-ve-xuat-xu-hang-hoa\?lang=en"/);
...
test("should expose compare-source links and raw source routes on document pages", async () => {
  ...
  try {
    ...
    assert.match(detailHtml, /Nguồn đối chiếu/);
    assert.match(detailHtml, /Trang toàn văn chính thức/);
