...
additive `product_identity` field per CO API request 2026-05-07.
            ...
            cur.execute(
                """insert into hub.bcct_rows
                   (client_id, transaction_key, line_no, declaration_no,
                    ...
                    customs_code, internal_code, goods_name, invoice_ref,
                    payload)
                   values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s::jsonb)
                """,
                (cid, f"{decl}-{line}", line, decl, dtype, direction,
                 regdate, code, code, gname, invref, "{}"),
            )
            # NULL product_identity — exercises lazy-fill path on read.
...
def test_bcct_default_omits_product_identity(seeded):
    """Per D7: /bcct default include_product_identity=false."""
    r = _client().get("/v1/hub/bcct", params={"client_id": seeded["client_id"]})
    assert r.status_code == 200
    items = r.json()["items"]
    assert items
    for it in items:
        assert "product_identity" not in it


def test_bcct_with_include_returns_product_identity(seeded):
    r = _client().get(
        "/v1/hub/bcct",
        params={"client_id": seeded["client_id"], "include_product_identity": "true"},
    )
    assert r.status_code == 200
    items = r.json()["items"]
    by_decl = {it["declaration_no"]: it for it in items}
    assert "product_identity" in by_decl["DECLAA"]
    pid_a = by_decl["DECLAA"]["product_identity"]
...
def test_invoice_matches_default_includes_product_identity(seeded):
    """Per D7: /invoice-matches default include_product_identity=true."""
    r = _client().get(
        "/v1/hub/bcct/invoice-matches",
        params={
    ...
    assert items, items
    for it in items:
        assert "product_identity" in it
        assert it["product_identity"]["line_key"]["client_id"] == seeded["client_id"]
...
def test_invoice_matches_explicit_false_omits(seeded):
    r = _client().get(
        ...
        params={
            ...
            "invoice_no": seeded["invoice_no"],
            "declaration_types": "E42",
            "include_product_identity": "false",
        },
    )
    assert r.status_code == 200
    items = r.json()["items"]
...
def test_invalid_candidate_limit_returns_400(seeded):
    r = _client().get(
        ...
        params={
            ...
            "product_identity_candidate_limit": "999",
        },
    )
    assert r.status_code == 400
    assert "invalid_product_identity_candidate_limit" in r.json()["detail"]
...
def test_non_integer_candidate_limit_returns_400(seeded):
    ...
    r = _client().get(
        ...
        params={
            "client_id": seeded["client_id"],
            "include_product_identity": "true",
            "product_identity_candidate_limit": "abc",
        },
    )
    assert r.status_code == 400
    assert "invalid_product_identity_candidate_limit" in r.json()["detail"]
...
def test_unknown_client_returns_404_with_pid_field(seeded):
    r = _client().get(
        "/v1/hub/bcct",
        params={"client_id": "nope-nope-nope", "include_product_identity": "true"},
    )
    assert r.status_code == 404


# ─── Lazy-fill (NULL product_identity rows) ──────────────────────────


def test_lazy_fill_resolves_at_read_time(seeded):
    """All seeded rows have product_identity=NULL in DB. Read endpoint
    resolves on the fly when include_product_identity=true."""
    ...
    items = r.json()["items"]
    # All should have a product_identity now (computed at read time).
    assert all("product_identity" in it for it in items)
    # Resolved ones have non-null bom_product_code.
    a = next(it for it in items if it["declaration_no"] == "DECLAA")
    assert a["product_identity"]["resolved_code"] == "PV01.0117500"
    assert a["product_identity"]["bom_product_code"] == "PV01.0117500"
