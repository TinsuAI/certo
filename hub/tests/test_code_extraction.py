...
def test_bcct_placeholder_is_config_not_hardcoded():
    """A client whose placeholder is '-' gets the same skip for '-' rows."""
    row = {"customs_code": "-", "goods_name": "anything (019.0079001)"}
    out = candidates_from_bcct_row(row, rules=[_paren_rule()],
                                    has_dual_system=True,
                                    placeholders=("-",))
...
def test_bcct_dot_is_real_code_without_placeholder_config():
    """No placeholder config → '.' is an ordinary customs_code, not missing."""
    row = {"customs_code": ".", "goods_name": "anything (019.0079001)"}
    out = candidates_from_bcct_row(row, rules=[_paren_rule()],
                                    has_dual_system=True,
                                    placeholders=())
    assert sorted(out) == sorted([(".", "hq"), ("019.0079001", "nb")])
...
def test_bcct_dot_strips_to_skip():
    """Whitespace-padded placeholder treated the same as the bare one."""
