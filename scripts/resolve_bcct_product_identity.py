"""Backfill script for `hub.bcct_rows.product_identity`.
...
CLI:
    ...
    uv run python -m scripts.resolve_bcct_product_identity --client-id growatt-vn

    # Recompute ALL rows for a client (drops cached parser_version).
    uv run python -m scripts.resolve_bcct_product_identity --client-id growatt-vn --recompute

    # Dry-run (no writes).
    uv run python -m scripts.resolve_bcct_product_identity --client-id growatt-vn --dry-run

    # All clients.
    uv run python -m scripts.resolve_bcct_product_identity --all-clients
"""
...
def resolve_for_client(client_id: str, *, recompute: bool, dry_run: bool,
    ...
    """Walk every BCCT row for a client, computing product_identity.
    ...
    with connect() as conn:
        with conn.cursor() as cur:
            ...
            where_clause = "" if recompute else " and product_identity is null"
            cur.execute(
                f"""
        ...
        if dry_run:
            for txkey, line_no, decl, customs, internal, gname in rows:
                pid = resolve_product_identity({
                    "transaction_key": txkey, "line_no": line_no,
                    "declaration_no": decl, "customs_code": customs,
                    "internal_code": internal, "goods_name": gname,
                }, ctx=ctx)
