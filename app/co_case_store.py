...
def update_case_record(client: dict, case: dict) -> dict:
    ...
    with case_lock(client["id"]):
        ...
        # NOTE: this list is a whitelist — a key missing from it is silently dropped on
        # every save (that is how the lô-hàng criterion choice first vanished).
        for key in ["criteria_choice", "origin_product_order", "origin_sheet_states", "bang_ke_column9_mode_override", "bom_artifact_id", "bom_version_id", "bom_product_artifact_overrides", "bom_product_version_overrides", "bom_snapshot", "origin_snapshot", "source_snapshot", "source_invoice_matches"]:
            if key in case:
...
def case_from_record(base_case: dict, client: dict, record: dict) -> dict:
    ...
    # Whitelist — a key missing here never reaches a render or a route, however well it
    # was persisted (that is how `criteria_choice` first went invisible).
    for key in ["criteria_choice", "origin_product_order", "origin_sheet_states", "bang_ke_column9_mode_override", "bom_artifact_id", "bom_version_id", "bom_product_artifact_overrides", "bom_product_version_overrides", "bom_snapshot", "origin_snapshot", "source_snapshot", "source_invoice_matches"]:
        if key in record:
